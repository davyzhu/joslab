...
//
// in gdt[NCPU+5], 5 is for NULL,KT,KD,UT,UD, NCPU is for percpu TSS
struct Segdesc gdt[NCPU + 5] =
{
  ...
  // 0x8>>3 = 0x1, GD_KT is offset in bytes, used in asm
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
  ...
  if (elfhdr->e_magic != ELF_MAGIC)
    panic("elf magic is not correct\n");
  ph = (struct Proghdr *) ((uint8_t *) elfhdr + elfhdr->e_phoff);
	eph = ph + elfhdr->e_phnum;

  // use new env's va for convience
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
  ...
  // test it after FS pass all basic test
  if (type == ENV_TYPE_FS) {
    e->env_tf.tf_eflags |= FL_IOPL_3; // CPL <= IOPL
  }
}
