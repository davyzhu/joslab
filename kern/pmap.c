...
i386_detect_memory(void)
{
	...
	npages_basemem = (nvram_read(NVRAM_BASELO) * 1024) / PGSIZE; //160
	npages_extmem = (nvram_read(NVRAM_EXTLO) * 1024) / PGSIZE; //16383
	...
	cprintf("npages basemem: %u, extmem %u, all %u\n", 
		npages_basemem, npages_extmem, npages);

	cprintf("Physical memory: %uK available, base = %uK, extended = %uK\n",
		npages * PGSIZE / 1024, //66556
		npages_basemem * PGSIZE / 1024, //640
		npages_extmem * PGSIZE / 1024); //65532
}
...
boot_alloc(uint32_t n)
{
  ...
  if(!n) return nextfree;
	...
	// see xv6:kalloc.c:enter_alloc 

	nextfree += n_pages*PGSIZE;
	
	/* 
   * cprintf("boot_alloc: n 0x%x, end 0x%x, result 0x%x, nextfree 0x%x\n",
	 * 	n, end, result, nextfree);
   */
	return result;
}
...
mem_init(void)
{
   ...
   * uint32_t i;
	 ...
	 * 	assert((lpa = (check_va2pa(kern_pgdir, UPAGES + i))) == (rpa = PADDR(pages) + i) ||
   *          cprintf("UPAGES+i %x lpa %x rpa %x\n", UPAGES+i, lpa, rpa));
   */
...
page_alloc(int alloc_flags)
{
  ...
  pp = page_free_list;
  //if (!pp) cprintf("pp is NULL\n");
  page_free_list = page_free_list->pp_link;
  //if (!page_free_list) cprintf("page_free_list is NULL\n");
...
check_page_free_list(bool only_low_memory)
{
  ...
  //cprintf("finish check_page_free_list\n");
}
...
check_kern_pgdir(void)
{
	uint32_t i, n;
	pde_t *pgdir;
  physaddr_t lpa, rpa; // left and right hand physical addr in assert
	...
	n = ROUNDUP(npages*sizeof(struct Page), PGSIZE);
	for (i = 0; i < n; i += PGSIZE)
		assert((lpa = (check_va2pa(pgdir, UPAGES + i))) == (rpa = PADDR(pages) + i) ||
           cprintf("i %x lpa %x rpa %x\n", i, lpa, rpa));

  cprintf("check pages array pass!\n");
  ...
  cprintf("check kernel stack pass!\n");

	// check phys mem
	for (i = 0; i < npages * PGSIZE; i += PGSIZE)
		assert((lpa = check_va2pa(pgdir, KERNBASE + i)) == i ||
           cprintf("lpa %x, rpa %x PGSIZE %x\n", lpa, i, PGSIZE));

  cprintf("check phys mem pass!\n");
...
check_page(void)
{
	...
	page_free(pp0);
	assert(page_insert(kern_pgdir, pp1, 0x0, PTE_W) == 0); // wrong1
